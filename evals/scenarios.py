...
SCENARIOS: tuple[Scenario, ...] = (
    ...
    Scenario(
        id="changes_mind",
        description="Changes the day at the read-back: only the final choice may be saved.",
        persona=(
        ...
        facts=COMMON_FACTS,
        behavior=(
            f"{NUMBER_RULE} Когда администратор ПЕРВЫЙ раз зачитает данные заявки, скажи, что "
            "передумал: лучше в понедельник, утром. Другие данные не меняй. Когда зачитают "
            "исправленную заявку, подтверди. На вопрос, нужна ли ещё помощь, скажи, что нет, "
            "и попрощайся."
        ),
        caller_phone=CALLER_ID,
        checks=(
    ...
    Scenario(
        ...
        description="Wants something that is not on the price list («оклеить фары»).",
        persona=(
            "Ты хочешь оклеить фары цветной декоративной плёнкой. Такой услуги ты в списке не "
            "знаешь и не уверен, делают ли её. Записаться ты хочешь в ближайшую субботу, после "
            "обеда."
        ),
        facts=COMMON_FACTS,
        behavior=(
            f"{NUMBER_RULE} Если администратор скажет, что уточнит детали и цену у мастера, "
            "согласись. Точное время не называй. Когда зачитают заявку, подтверди. На вопрос, "
    ...
    Scenario(
        ...
        behavior=(
            ...
            "резко скажи «всё, надоели» и положи трубку."
        ),
        caller_phone=CALLER_ID,
        checks=(
            c.no_bookings(),
