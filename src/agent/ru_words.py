...
# Nominative -> genitive, word by word, for the numerals `cardinal` produces («от десяти тысяч»).
_GENITIVE = {
    "ноль": "ноля",
    "один": "одного",
    "одна": "одной",
...
def cardinal_genitive(n: int) -> str:
    """0..999999 in the genitive: 10000 -> 'десяти тысяч', 3500 -> 'трёх тысяч пятисот'."""
    return " ".join(_GENITIVE[word] for word in cardinal(n).split())


def genitive_noun(n: int, singular: str, plural: str) -> str:
    """The noun after a genitive numeral: 'от двадцати одного рубля', 'до пяти рублей'."""
